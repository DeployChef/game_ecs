...
public class Run
{
    ...
    public void AddHandScore(int points)
    {
        ...
        {
            // Если не достигли цели, проверяем проигрыш
            // (может быть, мы исчерпали все возможности)
            CheckGameOver();
        }
    }
    ...
    /// 
    /// Примечание: Сбросы не помогают набрать очки, поэтому если нельзя играть и цель не достигнута,
    /// это проигрыш, даже если можно еще сбрасывать.
    /// </summary>
    public void CheckGameOver()
    {
        ...
        // Проигрыш: не можем играть и не достигли цели
        // (сбросы не помогают набрать очки, поэтому они не учитываются)
        if (!RoundState.CanPlayHand && !RoundState.IsComplete)
        {
            State = RunState.Lost;
        }
    }
